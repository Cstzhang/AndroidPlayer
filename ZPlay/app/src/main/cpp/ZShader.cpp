...
static GLuint InitShader(const char *code,GLint type)
{
    ...
    if(status == 0)
    {
        ZLOGE("glCompileShader failed!");
        return 0;
    }
    ZLOGI("glCompileShader success!");
    return sh;
}
...
bool ZShader::Init(ZShaderType type)
{
    ...
    ZLOGI("InitShader GL_VERTEX_SHADER success!");

    //片元yuv420 shader初始化
    switch (type)
    {
        case ZSHADER_YUV420P:
    ...
    ZLOGI("InitShader GL_FRAGMENT_SHADER success!");
